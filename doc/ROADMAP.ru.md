...
### 0.3.1 Адаптеры `dio` и кастомные клиенты ⚠️ Частично
...
- ✅ Поддержка `http.adapter` в конфиге:
  ...
  - ⚠️ `custom` — инфраструктура есть, но генерация кода для custom адаптеров не реализована.
- ❌ Для `custom`:
  - ❌ Генерация кода, ожидающего, что пользователь сам передаст экземпляр адаптера
    в `ApiClientConfig` — **TODO**.
