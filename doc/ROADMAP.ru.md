...
2. **Реализовать реальный `ModelsResolver`** (v0.4.2):
   - Чтение `dart_swagger_to_models.yaml` и кеша для поиска моделей.
   - Генерация методов с реальными типами вместо `Map<String, dynamic>`.
   - Критично для типобезопасности клиента.

3. **Добавить поддержку `application/x-www-form-urlencoded`** (v0.2.2):
   - Расширить `_hasRequestBody` для поддержки form‑данных.
   - Генерация правильной сериализации для form‑параметров.
...
3. 🎯 **v0.4.2** — реализовать реальный `ModelsResolver` (критично для типобезопасности)
4. 🔄 **v0.2.2** — добавить `application/x-www-form-urlencoded` (расширяет покрытие)
